...
Mouse Control stands on years of public Linux input and mouse-protocol research.
...
modules were introduced as Mouse Control files in Git history. Their known
...
Mouse Control or substitutes for advice from qualified counsel.

## Protocol and research credits

| Project or source | Contribution represented here | Current Mouse Control location | Public license / review state |
| --- | --- | --- | --- |
| [OpenRazer](https://github.com/openrazer/openrazer) | Razer 90-byte RPC envelope, command behavior, exact-product protocol research, and physical reports. | `src/mouse_control/native_razer.py`; `protocol_repertoire.py` | GitHub currently reports GPL-2.0. Mouse Control has no OpenRazer runtime dependency or vendored OpenRazer file. Preserve upstream notices if code or expressive tables are ever imported. |
...
| [OpenSharkX11](https://github.com/clevim/OpenSharkX11) | Attack Shark X11 report shapes, safety warning for report `0x0b`, and hardware observations. | `src/mouse_control/protocol_repertoire.py` | Upstream describes itself as MIT. Mouse Control records recognition facts and keeps all generic writes disabled. |
| [OpenMouse mouse-protocol](https://github.com/OpenMouse-Project/mouse-protocol) | Attack Shark X11 and MCHOSE transaction/reply research. | `src/mouse_control/protocol_repertoire.py`; Razer source comment | No top-level license file was identified by this audit. Treat protocol references as citation-only until a maintainer records a pinned upstream license and confirms the scope of any adaptation. |
...
| Linux HID/input maintainers, the [HID Usage Tables](https://usb.org/document-library/hid-usage-tables-15), and [libevdev](https://gitlab.freedesktop.org/libevdev/libevdev) | Platform APIs, standard HID semantics, and evdev behavior on which Mouse Control operates. | HID/evdev integration throughout `src/mouse_control` | No Linux, HID-tools, or libevdev source files are bundled. Standards and kernel interfaces are not treated as vendor-protocol write authority. |
...
Mouse Control distributes its own GPL-3.0-or-later source and the repository's
